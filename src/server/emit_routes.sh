...
# handle_emit_server_routes generates individual route files for the server.
handle_emit_server_routes() {
	out_dir="${1:-routes}"
	export out_dir
	...
	jq -r -f "${LIBSCRIPT_ROOT_DIR}/src/server/emit_routes.jq" "${ast}" | sh
}
