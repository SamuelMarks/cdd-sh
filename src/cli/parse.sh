...
handle_parse_cli() {
	...
	END {
	printf "INFO:{\"version\":\"%s\", \"title\":\"%s\", \"summary\":\"%s\", \"description\":\"%s\", \"termsOfService\":\"%s\", \"license\":{\"name\":\"%s\", \"url\": \"%s\", \"identifier\": \"%s\"}, \"contact\":{\"name\":\"%s\",\"email\":\"%s\", \"url\": \"%s\"}}\n", version, title, summary, desc, tos, license, lurl, lid, cname, cemail, curl
	printf "DIALECT:{\"jsonSchemaDialect\": \"%s\"}\n", dialect
	if (openapi_json != "") { printf "OPENAPI:%s\n", openapi_json }
	...
	for (o in ops) {
	 if (!first) printf ", "
	 first=0
	 printf "\"/cli/%s\": { \"post\": { \"operationId\": \"%s\", \"parameters\": [", o, o
	 pfirst=1
	 for (f in flags) {
	   ...
	   if (parts[1] == o) {
	     if (!pfirst) printf ", "
	     pfirst=0
	     printf "{\"name\": \"%s\", \"in\": \"query\"}", parts[2]
	   }
	 }
	 printf "] } }"
	}
	...
	awk '/^INFO:/ { print substr($0, 6) }' <all_temp.txt >cli_info.json || true
	awk '/^DIALECT:/ { print substr($0, 9) }' <all_temp.txt >cli_dialect.json || true
	awk '/^OPENAPI:/ { print substr($0, 9) }' <all_temp.txt >cli_openapi.json || true
	awk '/^PATHS:/ { print substr($0, 7) }' <all_temp.txt >cli_paths.json || true
	rm -f all_temp.txt
