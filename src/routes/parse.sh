...
handle_parse_routes() {
  ...
  if [ ! -f "${file_path}" ]; then
    printf "Error: routes file not found at %s\n" "${file_path}" >&2
    return 1
  fi
  ...
  awk '
    ...
    /^# @method / {
      if (in_webhook) method = tolower($3)
    }
    /^# @description / {
      if (in_webhook) summary = substr($0, 16)
      if (in_func) summary = substr($0, 16)
    }
    /^handle_webhook_/ {
      if (in_webhook) {
        printf "{\"name\": \"%s\", \"method\": \"%s\", \"summary\": \"%s\"}\n", webhookName, method, summary > "webhooks_temp.jsonl"
        in_webhook = 0
      }
    }
    /^# @function / {
    ...
    /^# @param/ {
      if (in_func) {
        if ($3 == "requestBody" || $4 == "requestBody") {
          ...
          gsub(/[\(\)]/, "", p_in)
          idx = index($0, "- ")
          if (idx > 0) p_desc = substr($0, idx + 2)
          else p_desc = ""
          params[param_count, "name"] = p_name
          params[param_count, "in"] = p_in
          params[param_count, "desc"] = p_desc
          param_count++
        }
      }
    }
    /^[ \t]*url=/ {
      if (in_func) {
        ...
        for (i=0; i<param_count; i++) {
          if (params[i, "in"] == "path") {
            orig_name = params[i, "name"]
            var_name = orig_name
            gsub(/-/, "_", var_name)
            # handle matrix, label, simple style formats replacement fallback
            gsub("{" var_name "}", "{" orig_name "}", url)
          }
        }
        path = url
      }
    ...
    /curl -s -X/ {
      if (in_func) {
        for (i=1; i<=NF; i++) {
          if ($i == "-X") method = tolower($(i+1))
        }
        printf "{\"path\": \"%s\", \"method\": \"%s\", \"operationId\": \"%s\", \"summary\": \"%s\"", path, method, operationId, summary > "ops_temp.jsonl"
        if (param_count > 0) {
  ...
  if [ -f "webhooks_temp.jsonl" ]; then
    ...
    ' "webhooks_temp.jsonl" > parsed_webhooks.json
